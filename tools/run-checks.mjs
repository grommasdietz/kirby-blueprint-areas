...
process.stdout.write("\nCustom API slug browser test\n");
const apiSlugBrowser = spawnSync(pnpm, ["run", "test:browser:api-slug"], {
  cwd: process.cwd(),
  stdio: "inherit",
  env: process.env,
});

if (apiSlugBrowser.error || apiSlugBrowser.status !== 0) {
  process.stdout.write("✗ Custom API slug browser test\n");
  process.stdout.write("Detailed report: pnpm run test:browser:report\n");
  process.exit(apiSlugBrowser.status ?? 1);
}
